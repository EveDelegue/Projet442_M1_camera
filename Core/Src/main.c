...
void StartDefaultTask(void const * argument)
{
  ...
  MX_USB_HOST_Init();
  /* USER CODE BEGIN 5 */
  int etat_prec=0,etat=0;
  char text[50]={};
  static TS_StateTypeDef  TS_State;
  int* mon_pointeur;
  int i = 0;int j=0;
  int off = 240*160*4-4;
  uint32_t tableau1[480],tableau2[480];
  int tableau_actif;
  int seuilnoir = 6,seuilblanc = 9;
  /* Infinite loop */
  for(;;)
  {
	  switch (etat){
	  		...
	  		etat_prec = 0;
	  		    ...
	  		    mon_pointeur = 0xC0260000 + 4*i; //+ off;//mon_pointeur = LCD_FB_START_ADDRESS + 4*270*i;
	  		    tableau1[2*i] = ((*mon_pointeur&0x0000F000)>>12); //A
	  		    tableau1[2*i+1] = ((*mon_pointeur&0xF0000000)>>28);

	  		    if (tableau1[2*i]>seuilblanc){
	  		    	tableau2[2*i] = 1;
	  		    }
	  		    else {
	  		    	if (tableau1[2*i]<seuilnoir){
	  		    		    	tableau2[2*i] = 0;
	  		    		    }
	  		    else{
	  		    	tableau2[2*i] = 2;}
	  		    }
	  		    if (tableau1[2*i+1]>seuilblanc){
	  		    		    	tableau2[2*i+1] = 1;
	  		    		    }
	  		    		    else {
	  		    		    	if (tableau1[2*i+1]<seuilnoir){
	  		    		    		    	tableau2[2*i+1] = 0;
	  		    		    		    }
	  		    		    else{
	  		    		    	tableau2[2*i+1] = 2;}
	  		    ...
	  		    sprintf(text,"luminosite : %x",(*mon_pointeur&0xF0000000)>>28);
	  		    BSP_LCD_DisplayStringAtLine(5,(uint8_t*) text);
	  		    *mon_pointeur = 0x0000FFFF;
	  		...
	  		case 2 :
	  			...
	  			BSP_TS_GetState(&TS_State);
	  				  	...
	  				  	sprintf(text,"colonne 0 val %d",tableau1[0]);
	  				  	BSP_LCD_DisplayStringAtLine(1,(uint8_t*) text);
	  				  sprintf(text,"colonne 239 val %d",tableau1[239]);
	  				  BSP_LCD_DisplayStringAtLine(2,(uint8_t*) text);
	  				  sprintf(text,"colonne 470 val %d",tableau1[470]);
	  				  BSP_LCD_DisplayStringAtLine(3,(uint8_t*) text);

	  			break;

	  		case 1 :
	  			mon_pointeur = LCD_FB_START_ADDRESS + 4*i + off;
	  			*mon_pointeur = 0x0000FFFF;
